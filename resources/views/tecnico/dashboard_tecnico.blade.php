...
$realizadoPrio = (bool) ($ent->detalleTecnico?->tec_realizado ?? false);
            ...
            <div style="background:{{ $realizadoPrio ? '#f0fdf4' : '#fff8f8' }}; border:1px solid {{ $realizadoPrio ? 'rgba(22,163,74,0.25)' : 'rgba(220,38,38,0.15)' }}; border-radius:10px; padding:10px 12px; transition:box-shadow 0.2s;"
...
<div style="display:grid; grid-template-columns:repeat(3,1fr); gap:14px; margin-bottom:14px;">
    ...
    {{-- 1. Total técnica --}}
    <a href="{{ route('tecnico.organizaciones') }}" class="card-stat" style="text-decoration:none; color:inherit; background:linear-gradient(135deg,#E8834A,#F5A623); border-radius:14px; padding:18px 20px; position:relative; overflow:hidden; box-shadow:0 4px 12px rgba(232,131,74,0.35);">
    ...
    {{-- 2. Enviados --}}
    <a href="{{ route('tecnico.organizaciones', ['estado' => 'enviado']) }}" class="card-stat" style="text-decoration:none; color:inherit; background:linear-gradient(135deg,#4A7C59,#6BAF7A); border-radius:14px; padding:18px 20px; position:relative; overflow:hidden; box-shadow:0 4px 12px rgba(74,124,89,0.35);">
        <div style="position:absolute;top:14px;right:16px;opacity:0.55;">
            <svg width="36" height="36" fill="none" stroke="#fff" stroke-width="1.5" viewBox="0 0 24 24">
    ...
    {{-- 3. Por imprimir --}}
    <a href="{{ route('tecnico.organizaciones', ['estado' => 'por_imprimir']) }}" class="card-stat" style="text-decoration:none; color:inherit; background:linear-gradient(135deg,#5B9EC9,#7BBDE0); border-radius:14px; padding:18px 20px; position:relative; overflow:hidden; box-shadow:0 4px 12px rgba(91,158,201,0.35);">
        <div style="position:absolute;top:14px;right:16px;opacity:0.55;">
            <svg width="36" height="36" fill="none" stroke="#fff" stroke-width="1.5" viewBox="0 0 24 24">
    ...
    {{-- 4. Impresos --}}
    <a href="{{ route('tecnico.organizaciones', ['estado' => 'impreso']) }}" class="card-stat" style="text-decoration:none; color:inherit; background:linear-gradient(135deg,#7A8A95,#9BAAB5); border-radius:14px; padding:18px 20px; position:relative; overflow:hidden; box-shadow:0 4px 12px rgba(46,107,138,0.35);">
        <div style="position:absolute;top:14px;right:16px;opacity:0.55;">
            <svg width="36" height="36" fill="none" stroke="#fff" stroke-width="1.5" viewBox="0 0 24 24">
    ...
    {{-- 5. Sin fecha --}}
    <a href="{{ route('tecnico.organizaciones', ['estado' => 'sin_fecha']) }}" class="card-stat" style="text-decoration:none; color:inherit; background:linear-gradient(135deg,#8A6B2E,#B59040); border-radius:14px; padding:18px 20px; position:relative; overflow:hidden; box-shadow:0 4px 12px rgba(138,107,46,0.35);">
        <div style="position:absolute;top:14px;right:16px;opacity:0.55;">
            <svg width="36" height="36" fill="none" stroke="#fff" stroke-width="1.5" viewBox="0 0 24 24">
    ...
    {{-- 6. Pendientes --}}
    <a href="{{ route('tecnico.organizaciones', ['estado' => 'pendiente']) }}" class="card-stat" style="text-decoration:none; color:inherit; background:linear-gradient(135deg,#C8A020,#E0BC40); border-radius:14px; padding:18px 20px; position:relative; overflow:hidden; box-shadow:0 4px 12px rgba(200,160,32,0.35);">
        <div style="position:absolute;top:14px;right:16px;opacity:0.55;">
            <svg width="36" height="36" fill="none" stroke="#fff" stroke-width="1.5" viewBox="0 0 24 24">
...
<div style="background:rgba(255,255,255,0.75); backdrop-filter:blur(12px); border:1px solid rgba(255,255,255,0.9); border-top:none; border-radius:0 0 16px 16px; box-shadow:0 20px 60px rgba(240,240,241,0.15); margin-bottom:40px;">
    <table style="width:100%; border-collapse:collapse; font-size:11px;">
        <tbody>
            ...
            <tr style="border-bottom:1px solid #f3f4f6; cursor:pointer;" onclick="window.location='{{ route('tecnico.organizacion.edit', $entrada->id) }}?{{ http_build_query(['volver' => request()->fullUrl()]) }}'" onmouseover="this.style.background='rgba(232,131,74,0.14)'" onmouseout="this.style.background='transparent'">
