...
<div class="px-2 py-2">
    <div style="max-width:860px; margin:0 auto;">
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px;">
            <div>
                <a href="<?php echo e(route('asesor.borrador.index')); ?>" style="font-size:12px; color:#94a3b8; text-decoration:none;">← Volver</a>
                <h2 style="font-size:16px; font-weight:700; color:#1e293b; margin:4px 0 0;"><?php echo e($borrador->nombre_organizacion); ?></h2>
                <p style="font-size:12px; color:#94a3b8; margin:2px 0 0;">
                    ...
                    <?php if($borrador->telefono_representante): ?> · <?php echo e($borrador->telefono_representante); ?> <?php endif; ?>
                </p>
            </div>
            <div style="display:flex; gap:8px; align-items:center;">
                <?php if($borrador->estado === 'activo'): ?>
                ...
                <form method="POST" action="<?php echo e(route('asesor.borrador.destroy', $borrador->id)); ?>"
        ...
        <?php if($borrador->notas_generales): ?>
        <div style="background:#fefce8; border:1px solid #fde68a; border-radius:12px; padding:14px 18px; margin-bottom:14px;">
            ...
            <p style="font-size:13px; color:#374151; margin:0;"><?php echo e($borrador->notas_generales); ?></p>
        </div>
        <?php endif; ?>
        ...
        <div style="background:#fff; border-radius:12px; border:1px solid #e5e7eb; padding:20px; margin-bottom:14px; box-shadow:0 1px 4px rgba(0,0,0,0.04);">
            ...
            <div id="form-tarea" style="display:none; background:#f8fafc; border-radius:10px; padding:16px; margin-bottom:16px; border:1px solid #e2e8f0;">
                <form method="POST" action="<?php echo e(route('asesor.borrador.storeTarea', $borrador->id)); ?>">
                    <?php echo csrf_field(); ?>
            ...
            <?php $__empty_1 = true; $__currentLoopData = $borrador->tareas; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tarea): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <div style="border-left:3px solid #2563eb; padding:10px 14px; margin-bottom:10px; background:#f8fafc; border-radius:0 8px 8px 0;">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:4px;">
                    <span style="font-size:12px; font-weight:600; color:#2563eb;"><?php echo e($tarea->tipo); ?></span>
                    <span style="font-size:11px; color:#94a3b8;"><?php echo e(\Carbon\Carbon::parse($tarea->fecha)->format('d/m/Y')); ?></span>
                </div>
                <p style="font-size:13px; color:#374151; margin:0;"><?php echo e($tarea->nota); ?></p>
            </div>
...
<div id="modal-mesa" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.4); z-index:50; align-items:center; justify-content:center;">
    <div style="background:white; border-radius:12px; padding:28px; max-width:440px; width:90%; box-shadow:0 20px 60px rgba(0,0,0,0.3);">
        ...
        <div style="background:#f8fafc; border-radius:8px; padding:14px; margin-bottom:18px; font-size:13px; color:#374151;">
            <div style="margin-bottom:6px;"><strong>Organización:</strong> <?php echo e($borrador->nombre_organizacion); ?></div>
            <div style="margin-bottom:6px;"><strong>Tipo:</strong> <?php echo e($borrador->tipo_organizacion ?? '—'); ?></div>
            <div><strong>Representante:</strong> <?php echo e($borrador->nombre_representante ?? '—'); ?></div>
        </div>
        ...
        <div style="display:flex; gap:10px; justify-content:flex-end;">
            ...
            <form method="POST" action="<?php echo e(route('asesor.borrador.enviar', $borrador->id)); ?>">
                <?php echo csrf_field(); ?>
                <button type="submit"
...
<?php endif; ?>
<?php /**PATH /var/www/html/resources/views/panel/asesor/borrador/show.blade.php ENDPATH**/ ?>